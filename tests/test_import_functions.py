...
class TestImportMetadata(unittest.TestCase):
    def test_import_well_formatted_metadata(self):
        with TemporaryDirectory() as temp_dir:
            db_map = create_diff_db_map(temp_dir)
    ...
    def test_import_repeated_metadata(self):
        with TemporaryDirectory() as temp_dir:
            db_map = create_diff_db_map(temp_dir)
    ...
    def test_import_nested_metadata(self):
        with TemporaryDirectory() as temp_dir:
            db_map = create_diff_db_map(temp_dir)
...
class TestImportEntityMetadata(unittest.TestCase):
    ...
    def populate(db_map):
        import_object_classes(db_map, ["object_class1"])
        import_objects(db_map, [("object_class1", "object1")])
