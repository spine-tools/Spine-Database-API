...
from .helpers import COLOR_RE, DisplayStatus, name_from_dimensions, name_from_elements
...
class ParameterGroupItem(MappedItemBase):
    ...
    fields = {
        ...
        "background_color": {"type": str, "value": "The colour of the background of the group."},
    }
    unique_keys = (("name",),)

    def validate_keys(self):
        super().validate_keys()
        color_field = "background_color"
        if (color := self[color_field]) is not None:
            if COLOR_RE.match(color) is None:
                raise SpineDBAPIError(f"invalid {color_field} for {self.item_type}")
