...
# TODO: are CheckConstraint working?
# TODO: What about ON CASCADE and ON UPDATE for fks???


def create_new_tables():
    """Creates entity_class_type and entity_class tables."""
    op.create_table(
        "entity_class_type",
    ...
    op.create_table(
        "entity_class",
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("type_id", sa.Integer, sa.ForeignKey("entity_class_type.id")),
        sa.Column("name", sa.Unicode(255), nullable=False, unique=True),
    ...
    op.create_table(
        ...
        sa.Column("entity_class_id", sa.Integer, sa.ForeignKey("temp_relationship_class.entity_class_id")),
    ...
    op.create_table(
        "entity",
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("type_id", sa.Integer, sa.ForeignKey("entity_type.id")),
        sa.Column("class_id", sa.Integer, sa.ForeignKey("entity_class.id")),
...
def insert_into_new_tables():
    ...
    op.bulk_insert(m.tables["entity_class"], entity_classes)
    # Id mappings
    obj_cls_to_ent_cls = {
    ...
    op.bulk_insert(m.tables["temp_relationship_class"], temp_relationship_classes)
    ...
    op.bulk_insert(m.tables["relationship_entity_class"], relationship_entity_classes)
    # entity level
    entities = [
    ...
    op.bulk_insert(m.tables["entity"], entities)
    # Id mappings
    obj_to_ent = {
    ...
    op.bulk_insert(m.tables["temp_relationship"], temp_relationships)
    ...
    op.bulk_insert(m.tables["relationship_entity"], relationship_entities)
    # Return id mappings
    return (obj_cls_to_ent_cls, rel_cls_to_ent_cls, obj_to_ent, rel_to_ent)
...
def alter_old_tables_before_update():
    with op.batch_alter_table("object_class", naming_convention=naming_convention) as batch_op:
        ...
        batch_op.create_check_constraint("ck_object_class_entity_class_type", sa.sql.column("type_id") == 1)
    with op.batch_alter_table("object", naming_convention=naming_convention) as batch_op:
        ...
        batch_op.create_check_constraint("ck_object_entity_type", sa.sql.column("type_id") == 1)
...
def alter_old_tables_after_update():
    ...
    with op.batch_alter_table("parameter_value") as batch_op:
        ...
        batch_op.drop_column("dummy_relationship_dimmension")


def upgrade():
    create_new_tables()
    obj_cls_to_ent_cls, rel_cls_to_ent_cls, obj_to_ent, rel_to_ent = insert_into_new_tables()
    alter_old_tables_before_update()
    update_old_tables(obj_cls_to_ent_cls, rel_cls_to_ent_cls, obj_to_ent, rel_to_ent)
    alter_old_tables_after_update()
