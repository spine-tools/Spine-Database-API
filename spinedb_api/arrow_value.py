...
def from_dict(loaded_value, value_type):
    ...
    Returns:
        pyarrow.StructArray: parsed value
    """
    if value_type == "array":
        ...
        return pyarrow.StructArray.from_arrays([x_array, y_array], names=[loaded_value.get("index_name", "i"), "y"])
    if value_type == "map":
...
def map_to_struct_array(loaded_value):
    ...
    return pyarrow.StructArray.from_arrays(x_arrays + [build_y_array(ys)], names=index_names + ["y"])
