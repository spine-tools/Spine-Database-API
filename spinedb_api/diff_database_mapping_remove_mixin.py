...
class DiffDatabaseMappingRemoveMixin:
    ...
    def remove_items(
        ...
        try:
            for tablename, ids in removed_diff_item_id.items():
                ...
                self.query(diff_class).filter(diff_class.id.in_(ids)).delete(synchronize_session=False)
            self.session.commit()
    ...
    def _removed_items(
        ...
        # object_class
        item_list = self.query(self.ObjectClass.id).filter(self.ObjectClass.id.in_(object_class_ids))
        diff_item_list = self.query(self.DiffObjectClass.id).filter(self.DiffObjectClass.id.in_(object_class_ids))
        self._remove_cascade_object_classes(
            [x.id for x in item_list], [x.id for x in diff_item_list], removed_item_id, removed_diff_item_id
        )
        # object
        item_list = self.query(self.Object.id).filter(self.Object.id.in_(object_ids))
        diff_item_list = self.query(self.DiffObject.id).filter(self.DiffObject.id.in_(object_ids))
        self._remove_cascade_objects(
            [x.id for x in item_list], [x.id for x in diff_item_list], removed_item_id, removed_diff_item_id
        )
        # relationship_class
        item_list = self.query(self.RelationshipClass.id).filter(self.RelationshipClass.id.in_(relationship_class_ids))
        ...
        # relationship
        item_list = self.query(self.Relationship.id).filter(self.Relationship.id.in_(relationship_ids))
        diff_item_list = self.query(self.DiffRelationship.id).filter(self.DiffRelationship.id.in_(relationship_ids))
    ...
    def _remove_cascade_object_classes(self, ids, diff_ids, removed_item_id, removed_diff_item_id):
        ...
        diff_item_list = self.query(self.DiffObject.id).filter(self.DiffObject.class_id.in_(ids + diff_ids))
        self._remove_cascade_objects(
            [x.id for x in item_list], [x.id for x in diff_item_list], removed_item_id, removed_diff_item_id
        )
        # relationship_class
        item_list = self.query(self.RelationshipClass.id).filter(self.RelationshipClass.object_class_id.in_(ids))
        diff_item_list = self.query(self.DiffRelationshipClass.id).filter(
        ...
        # parameter
        item_list = self.query(self.ParameterDefinition.id).filter(self.ParameterDefinition.object_class_id.in_(ids))
        diff_item_list = self.query(self.DiffParameterDefinition.id).filter(
    ...
    def _remove_cascade_objects(self, ids, diff_ids, removed_item_id, removed_diff_item_id):
        ...
        # relationship
        item_list = self.query(self.Relationship.id).filter(self.Relationship.object_id.in_(ids))
        ...
        # parameter_value
        item_list = self.query(self.ParameterValue.id).filter(self.ParameterValue.object_id.in_(ids))
        diff_item_list = self.query(self.DiffParameterValue.id).filter(
    ...
    def _remove_cascade_relationship_classes(self, ids, diff_ids, removed_item_id, removed_diff_item_id):
        ...
        # relationship
        item_list = self.query(self.Relationship.id).filter(self.Relationship.class_id.in_(ids))
        diff_item_list = self.query(self.DiffRelationship.id).filter(self.DiffRelationship.class_id.in_(ids + diff_ids))
        self._remove_cascade_relationships(
            [x.id for x in item_list], [x.id for x in diff_item_list], removed_item_id, removed_diff_item_id
        )
        # parameter
    ...
    def _remove_cascade_relationships(self, ids, diff_ids, removed_item_id, removed_diff_item_id):
        ...
        # parameter_value
        item_list = self.query(self.ParameterValue.id).filter(self.ParameterValue.relationship_id.in_(ids))
        diff_item_list = self.query(self.DiffParameterValue.id).filter(
