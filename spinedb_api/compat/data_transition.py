...
def make_columns(
    ...
    *,
    ...
    The parsing logic works recursively by traversing (rows) depth
    first.  Each call incrementally accumulates a cell/level of a
    record in the `idx_lvls` dictionary, once the traversal reaches a
    leaf node, the final record is appended to the list `res`.  The
    final result is also returned by the function, allowing for
    composition.
    ...
    # `make_columns` available to them, but nothing more.  They either
    ...
    def _from_pairs(data: Iterable[Iterable], fmt: _FmtIdx):
        ...
        for key, val in data:
            ...
            make_columns(val, _lvls, res, lvlname_base=lvlname_base)
    ...
    def _append_arr(arr: Iterable, fmt: _FmtIdx):
        ...
        # FIXME: nested loop
        for value in arr:
            _append_row({**idx_lvls, **fmt(index_name, value)})
    ...
    match json_doc:
        ...
        case {
            ...
            _append_row({**idx_lvls, **_fmt("value", data)})
        # values
